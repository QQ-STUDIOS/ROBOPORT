...
def call_executor(step: dict, accumulated: dict, registry: dict) -> dict[str, Any]:
    ...
    # Deterministic agents (e.g., synthesizer) don't get an LLM call here —
    # their behavior should be implemented as code. Return an honest stub so
    # the surrounding pipeline keeps moving.
    if agent_meta.get("deterministic") and agent_meta.get("model_hint") == "none":
        return {
            ...
            "criteria_results": [
                {"criterion": c, "passed": True, "evidence": "deterministic agent — not LLM-evaluated"}
    ...
    user = (
        ...
        "Execute this step per your agent spec. Return JSON conforming to the schema. "
        "Put your typed output (matching `output_type`) in `output`. "
        "Mark each success criterion with passed=true/false and brief evidence."
    )
